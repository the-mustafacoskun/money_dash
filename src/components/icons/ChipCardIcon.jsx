import React from 'react';

export const ChipCardIcon = ({ 
  size = 35, 
  className = "" 
}) => {
  // Senin paylaştığın birebir aynı görsel verisi
  const base64Image = "data:image/png;base64,";

  return (
    <svg 
      width={size} 
      height={size} 
      viewBox="0 0 35 35" 
      className={className}
    >
      <defs>
        {/* Maske oluşturuyoruz: Resmin şeklini alacak */}
        <mask id="chip-mask">
          <image 
            width="35" 
            height="35" 
            xlinkHref={base64Image} 
            style={{ filter: 'brightness(0) invert(1)' }} // Resmi beyaza çevirip maske yapıyoruz
          />
        </mask>
      </defs>
      
      {/* Bu dikdörtgen, maske sayesinde sadece resmin olduğu yerleri "currentColor" ile boyar */}
      <rect 
        width="35" 
        height="35" 
        fill="currentColor" 
        mask="url(#chip-mask)" 
      />
    </svg>
  );
};

