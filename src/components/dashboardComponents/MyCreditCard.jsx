...
export const MyCreditCard = ({card}) => {
    ...
    if (error) {
      return <div>Error loading card: {error.message}</div>;
    }
    ...
    <div className="flex flex-col p-6 gap-y-6 aspect-3/2 w-full max-w-87.5 bg-[linear-gradient(107.38deg,#4C49ED_2.61%,#0A06F4_101.2%)] rounded-3xl shadow-lg">
      <div className="flex justify-between items-center">
        <div className="flex flex-col">
          ...
          <span className="font-semibold text-[20px] text-white">{card.balance}</span>
        </div>
      ...
      <div className="grid grid-cols-2 gap-x-4">
        ...
        <span className="font-semibold text-[15px] text-white truncate">{card.holder}</span>
        <span className="font-semibold text-[15px] text-white">{card.expiry}</span>
      </div>
      
      <div className="flex justify-between items-center mt-auto  text-white">
        <span className="font-semibold text-[18px] md:text-[22px]">****  ****  ****  {card.number}</span>
        <MasterCardIcon />
      </div>
    </div>
  );
